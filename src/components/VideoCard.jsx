...
function VideoCard({displayData,setDeleteVideoResponse,insideCategory}) {
    ...
    const handleShow = async () =>{ 
      ...
      let timeStamp =new Intl.DateTimeFormat('en-US',{ 
        year:'numeric',month:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit'
      }).format(timeData)
   ...
   <Card draggable onDragStart={(e)=>dragStarted(e,displayData?.id)} className='shadow'>
    ...
    <Modal show={show} onHide={handleClose}>
        <ModalHeader closeButton>
            <ModalTitle>{displayData?.caption}</ModalTitle>
        </ModalHeader>
        <Modal.Body><iframe width="100%" height="400" src={`${displayData?.youtubeLink}?autoplay=1`}
